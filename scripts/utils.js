...
export const MODULE_ID = 'court-of-blades';
...
export class Utils {
  ...
  static async getAllItemsByType(item_type) {
    ...
    let compendium_content = await Promise.all(packs.map(e => { return (e.getDocuments()) }));
    compendium_items = compendium_content.flat();
  ...
  static async getSourcedItemsByType(item_type) {
    const populateFromCobCompendia = game.settings.get('court-of-blades', 'populateFromCobCompendia');
    const populateFromBitdCompendia = game.settings.get('court-of-blades', 'populateFromBitdCompendia');
    const populateFromOtherCompendia = game.settings.get('court-of-blades', 'populateFromOtherCompendia');
    const populateFromWorld = game.settings.get('court-of-blades', 'populateFromWorld');
    let limited_items = new Array(4);

    if (populateFromCobCompendia) {
      limited_items[0] = await game.packs.get("court-of-blades." + item_type).getDocuments();
    }

    if (populateFromBitdCompendia) {
      limited_items[1] = await game.packs.get("blades-in-the-dark." + item_type).getDocuments();
    }

    if (populateFromOtherCompendia) {
      let packs = game.packs.filter(e => e.metadata.name === item_type && e.metadata.packageName !== 'blades-in-the-dark' && e.metadata.packageName !== 'court-of-blades');
      let compendium_content = await Promise.all(packs.map(e => { return (e.getDocuments()) }));
      limited_items[2] = compendium_content.flat();
    }

    if (populateFromWorld) {
      if (item_type === "npc") {
        limited_items[3] = game.actors.filter(actor => actor.type === item_type);
      }
      else {
        limited_items[3] = game.items.filter(item => item.type === item_type);
      }
    }

    if (!(populateFromCobCompendia || populateFromBitdCompendia || populateFromOtherCompendia || populateFromWorld)) {
    ...
    if (limited_items) {
      limited_items = limited_items.flat()
  ...
  // TODO move from Utils to character sheet?
  /**
  ...
  static async toggleOwnership(state, actor, type, id) {
    if (type == "ability" || type == "crew_ability") {
      if (state) {
    ...
    else if (type == "item" || type == "crew_upgrade") {
      console.log("It's an item");
      // let item = actor.items.find(item => item.id === id);
      let equipped_items = await actor.getFlag('court-of-blades', 'equipped-items');
      if (!equipped_items) {
        equipped_items = [];
      }
      let item_blueprint;
      ...
      //   equipped_items = [id];
      // }
      await actor.setFlag('court-of-blades', 'equipped-items', equipped_items);
    }
  }
